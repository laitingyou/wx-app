...
const NIM = require('./sdk/NIM_Web_NIM_v5.1.0')
...
export default class IMEventHandler {    
    constructor(headers) { 
        ...
        app.globalData.nim = NIM.getInstance({
            // 初始化SDk
            // debug           : true,
            // 应对域名被封禁，https://nosupload.netease.im 和 https://nim.nosdn.127.net替代
            uploadUrl: 'https://nosupload.netease.im', 
    ...
    onMsg(msg) {
        ...
        // 发送收到消息更新消息
        app.globalData.subscriber.emit('RECEIVE_P2P_MESSAGE', {
            account: account,
            time: msg.time
        })
        if (app.globalData.currentChatTo === account) { //当前正在与此账户会话，禁止更新最近会话unread
            app.globalData.subscriber.emit('UPDATE_RECENT_CHAT_ON_MSG', {
                account: account,
                time: msg.time,
                text: msg.text,
                type
    ...
    onSysMsg(sysMsg) {
        ...
        if (msg.type === 'deleteMsg') {
            ...
            if (app.globalData.currentChatTo === account) { //当前正在与此账户会话，禁止更新最近会话unread
                app.globalData.subscriber.emit('UPDATE_RECENT_CHAT', {
                    account: account,
                    type: msg.type,
                    time: msg.time,
                    text: ''
            ...
            app.globalData.subscriber.emit('OPPOSITE_RECALL_WHEN_CHATTING', {
                account: account,
                time: msgTime,
                tip
            })
    ...
    onSessions(sessions) {
        ...
        setTimeout(() => {
            for (let i = 0; i < sessions.length; i++) {
                ...
                app.globalData.subscriber.emit('UPDATE_RECENT_CHAT_ON_SESSION', {
                    account: account,
                    time: msg.time,
                    text: msg.text,
                    type,
                ...
                if (session.unread != 0) { // 更新未读数
                    app.globalData.subscriber.emit('UPDATE_RECENT_CHAT_UNREAD_ON_SESSION', {
                        account: session.to,
    ...
    onUpdateSession(session) {
        ...
        if (session.unread == 0) { // 其他端已已读未读会话
            // 清除未读会话
            app.globalData.subscriber.emit('CLEAR_UNREAD_RECENTCHAT_UPDATESESSION', {
                account: session.to
            })
        }
    }
