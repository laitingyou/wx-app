...
Page({
    ...
    onLoad: function (options) {
        store.on('im_init', this, function() {
            this._init()
        })
        let neteaseIm = wx.getStorageSync('neteaseIm')        
        if (neteaseIm && !app.globalData.loginUser.account) {
            new IMEventHandler({ 
    ...
    sortChatList() {        
        if (this.data.chatList.length !== 0) {
            ...
            chatList.map(function(item){
                item.isSystem = /^(system_)/.test(item.account)
                return item
            })            
            // 记录总未读数
            let sumUnread = 0
    ...
    switchToChating(e) {
        ...
        wx.navigateTo({
            url: `/imPartials/pages/chating/chating?chatTo=${account}`,
        })
    },
    /**
     * 单击进入个人区域
