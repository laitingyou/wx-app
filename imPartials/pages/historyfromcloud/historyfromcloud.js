...
Page({
    ...
    onLoad: function(options) {
        ...
        this.setData({
            ...
            userLogo: app.globalData.loginUser['avatar'] || '/images/default-icon.png'
        })
    },
    /**
     * 下拉刷新钩子
    ...
    formatMsgs(msgs) {
        ...
        msgs.map((message, index) => {
            // 自定义消息 
            let content = !!message['content'] ? JSON.parse(message['content']) : {}
            // 要推过的消息
            let pushObj = {
                // type,
                text: message.text,
            ...
            // 类型 
            let type = ''
            if (message.type === 'custom' && content['type'] === 1) {
                type = '猜拳'
            } else if (message.type === 'custom' && content['type'] === 3) {
                type = '贴图表情'

            } else if (message.type === 'custom' && content['type'] === 'products'){   
                type = 'products'
                pushObj.bodies = content.bodies
            } else if (message.type === 'custom' && content['type'] === 'concern'){
                type = 'text'
            ...
            pushObj.nodes = nodes
            pushObj.type = type            
            pushObj.time = message.time
            pushObj.displayTimeHeader = displayTimeHeader
