import {
    ...
    deepClone,
    clickLogoJumpToCard
} from '../../../utils/imUtil.js'
 
// 云信的初始化方法等等
import IMEventHandler from '../../../utils/imeventhandler'

const {autoSayHello} = require('../../interface/eellyIm')
...
Page({
    data: {
        pageTitle: '',
        defaultUserLogo: '/images/loginDian.png',
   ...
   */
    onLoad: function (options) {
        ...
        if (!app.globalData.loginUser.account) {
            // 初始化聊天室
    ...
    _init(){
        ...
        let {chatTo} = this.data
        // 向服务端请求，让它发一条自定义消息
        this.sayHello(Number.parseInt(chatTo.split('_')[1]))
        ...
        if (chatTo === app.globalData['loginUser']['account']) {
            ...
            app.globalData.nim.getUser({
                account: chatTo,
                    ...
                    let defaultUserLogo = user.nick == '衣联直播消息' ? '/images/iconLive.png' : self.data.defaultUserLogo  
                    ...
                    if (user['avatar']) {
                        ...
                        chatToLogo = defaultUserLogo
                    }
                    self.setData({
                        chatToLogo,
                        loginAccountLogo: app.globalData.loginUser['avatar'] || defaultUserLogo
                    })
                }
            })
        }
        ...
        // 监听p2p消息
        app.globalData.subscriber.on('RECEIVE_P2P_MESSAGE', ({
            account,
            time
        }) => {
            if (self.data.chatTo !== account) { // 非当前聊天人消息
    ...
    /**
     * [sayHello 给服务发起请求，发三个的商品链接“自定义消息”过来
     * 请在登录后使用]
